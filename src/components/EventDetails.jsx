...
const EventDetails = () => {
    ...
    useEffect(() => {
        ...
        const calculateTimeLeft = () => {
            let targetDate;
            let currentLabel = 'Event Starts In';

            // Special logic for TechSprint Grand Finale (ID 4)
            if (event.id === 4) {
                const now = new Date();
                const codingDeadline = new Date('Jan 31, 2026 12:15:00');
                const pitchingDeadline = new Date('Jan 31, 2026 13:15:00');

                if (now < codingDeadline) {
                    targetDate = codingDeadline;
                    currentLabel = "Coding Round Ends In 👨‍💻";
                } else if (now < pitchingDeadline) {
                    targetDate = pitchingDeadline;
                    currentLabel = "Final Pitches End In 🎤";
                } else {
                    return {}; // Event Over
                }
            } else {
                // Default logic
                targetDate = new Date(`${event.date} 11:15:00`);
            }
            ...
            setCountdownLabel(currentLabel);
            return timeLeft;
        };
